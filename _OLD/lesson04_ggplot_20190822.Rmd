...
3. The components of a basic RMarkdown report
4. Markdown customisation options
...
![](../figures/ggplot_layers_ex_geom_layer.png)
...
![](../figures/ggplot_layers_ex_axes_layer.png)
...
![](../figures/ggplot_layers_ex_final.png)
...
To be completely honest, we could spend the entire semester on these eight packages. However, we will not be able to do that at this juncture. We are going to focus most of our time on `ggplot`, the `tibble` / `tidyr` / `readr` group, and `dplyr`. Also, I hope to touch one some of the components you need in order to understand the `purrr` package in the first week of October, but we won't use the package itself.
...
# My First ggplot
Now that you have an understanding of how the layered grammar of graphics works (in theory), we can build our own plot. Within the `tidyverse`, there is an example data set on car manufacturing specifics stored in the object `mpg` (because it is in the package, you won't see it in the "Environment" pane).
```{r mpg_data}
mpg
```

Don't worry about what this all means for now. Just think of this object as R's version of an Excel spreadsheet. Observations are in the rows, measurements / variables are in the columns, each column has a name, and each column stores one *class* / *type* of data.
...
> Exercise: Check the definitions and explinations of the other variables in the **help file** on the `mpg` object.
...
> Exercise: Run `ggplot(data = mpg)`. What does this look like?
...
> Exercise: Use the `str()` function to confirm that the `mpg` object has `data.frame` as one of its *classes*.
...
Now that we have created a canvas to paint on, we need to choose our proverbial "colour palette". That is, we need to specify which measurements contained in the `mpg` data set we are going to use, and how they will influence the blank canvas. Now, consider the second line of code: `aes(x = displ, y = hwy)`. Following what we know about function syntax, we can deconstruct this line as follows:
...
> 5. Add 3 and 5 together. Is there anything else different with the `+` function?
...
Check the help documentation for the `mpg` data set to find out which column measures the type of the car. We probably won't see "hybrid"" in this list, because we know that very few hybrid cars were in production in both 1998 and 2008. These cars would have all been classified as "compact" or "sub-compact".
...
> Exercise: Write the code necessary to create the above graph.
...
> Exercise: Write the code necessary to create the above graph. *Hint: the shape is not dependent on the data, so it is not an aesthetic. Think about where you should put the shape argument if you can't put it in the `aes()` function.*
...
> Exercise: Add comments to the code chunk used to make this graph. Explain what layer each line is adding to the graph, and how they change the output. *Hint: you may need to build this graph line-by-line and without function arguments, like we did the first `ggplot` graph, in order to see all of the changes.*
...
> 2. Make the labels for the `x`, `y`, and `colour` aesthetics more professional / presentable
...
> Exercise: Discuss the above figure with your neighbours. What information can you draw from this figure? What can be done to improve it?
