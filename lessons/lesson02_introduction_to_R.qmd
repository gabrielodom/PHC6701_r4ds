...
- Do:
    + the [pre-course learning assessment](https://gabrielodom.github.io/PHC6701_r4ds/) and email your answers to me in a Word document.
    + Install the Tidyverse package suite for R (paste this code into your Console: `install.packages("tidyverse")`, then press the `Enter` (Windows) / `return` (Mac) key; if RStudio prompts you to restart, do it).
...
::: {.callout-tip}
An *expression* in the R language is what we call a small line of code. All of the examples we have written so far have been "expressions". If you want to learn more, here is a [very advanced chapter](https://adv-r.hadley.nz/expressions.html) on the topic. But, in the most basic sence, "expressions" are the code you type.
:::
...
::: {.callout-note title="Exercises"}
## Exercises
1. Create a sequence from 1 to 100. How does the display of this vector differ from the display of the object `keypad`?
...
The `c()` function will put almost anything you want into a single vector, provided that they are all similar things. For instance, I can make a vector of my full name and terminal degrees:
```{r concat_names}
...
# Getting Help
Sometimes, you need to [get some help](https://www.youtube.com/watch?v=OzRuuFA6oUs) with R code. To get help on any function you know the name of, type `?` then the name (with no spaces). For example, to find the help file for the `round()` function, type
```{r eval=FALSE}
?round
```
