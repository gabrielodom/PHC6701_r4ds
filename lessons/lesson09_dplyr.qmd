...
- Watch: <https://www.youtube.com/playlist?list=PLHKz7LkI0oR2Huix3BcCUJDB_vGchd6NH>
- Read:  
    + The "Data Transformation" chapter of R4DS (<https://r4ds.hadley.nz/data-transform.html>)
    + The "Data Tidying" chapter of R4DS (<https://r4ds.hadley.nz/data-tidy.html>)
    + The "Joins" chapter of R4DS (<https://r4ds.hadley.nz/joins.html>)
...
- `left_join(A, B, by = "x")`: join matching rows from `B` to `A`. Missing rows in `B` will be padded with `NA`. The resulting data set will have the same number of rows as `A`.
- `right_join(A, B, by = "x")`: join matching rows from `A` to `B`. Missing rows in `A` will be padded with `NA`. The resulting data set will have the same number of rows as `B`.
...
There is a [Data Carpentry lesson](https://tavareshugo.github.io/r-intro-tidyverse-gapminder/08-joins) on `dplyr::` joins that may be helpful to you, and I copied this Venn diagram from that lesson.

![](../_figures/dplyr_joins_venn.svg)
