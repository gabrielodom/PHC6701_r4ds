...
## Combine the Results
We have a list of the numeric summaries of life expectancy for each of the `r length(countries_ls)` countries. Let's combine these results.

::: {.callout-note title="Exercise"}
## Exercise
In the last exercise, we found the summaries of all the features for each country. Can these results be combined into a single tibble? Why or why not?
:::


## Example: Back to Tornados
Finally, one of the most valuable applications of this technique is to import and analyze multiple data files all at once.
```{r}
