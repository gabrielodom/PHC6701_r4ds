...
If we want to inspect the internal **str**ucture of this list, we can see that the classes of our original atomic vectors have been preserved. There is no class coercion in a list.
```{r}
str(DrGabriel_ls)
```
...
> 2. Use the help files to read about the `state.abb` object I used. Did I create it? Where did it come from?
> 3. The `DrGabriel_ls` object itself contains an atomic vector of length 2. Is this still a 1-D object? Why or why not?
> 4. Update your ID card to a list format. Include an entry for some of your favourite foods as an atomic character vector.
...
![](../figures/vectors_data_frame.png)
...
> Exercise: Add the code to create the tibble above to your script from last lesson.
...
> 2. Check the help file of the `data.frame()` function to find out how to preserve character information. What option should you add to the code above?
...
> Exercise (OPTIONAL): Tibbles include a lot of meta-data about themselves when they are printed. Which of the above functions would you have to use to find out the same information about a regular data frame?
...
![](../figures/vectors_hadley_subset_pepper.png)
...
> Exercise: Subset the tibble by every other row and column.
...
How many heroes include the name "man" somewhere in their name?
```{r}
# Step 1: subset the names of the superheroes:
heroes_df[["name"]]

# Step 2: compare these names to the character strings "man":
str_detect(heroes_df[["name"]], "man")

# Step 3: subset the tibble to return only the heroes with "man" in their names
heroes_df[str_detect(heroes_df[["name"]], "man"), ]
```

> Exercises:
> 
> 1. Find all the heroes who's home city has the word "City" in it.
> 2. Why couldn't we find Spider-Man in the example above?
...
> 4. Use compound logic (recall the AND and OR symbols) to find the superheroes with less than average height with greater than average weight.
> 5. Recall that code-writing best practices include that lines should not be longer than 80 characters wide. Your solution for the last question was most likely longer than that. What can you do to make the lines shorter and more easily readible?
...
> 5. Extract from the tibble a column that doesn't exist, `"Publisher"` for instance. What happens?
...
### Symbol Subsetting
We have seen this behaviour before, when we created plots with `ggplot`. The `aes()` function took in `displ` and `hwy` as symbols, and looked for objects with those names in the supplied `mpg` data set. Because this is a very complex but powerful tool, we will return to it in a few lessons.


> Exercise: 
> 
> 1. Extract the non-existant `Publisher` column from the `heroes_df` tibble. What happens?
